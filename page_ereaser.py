...
def images_to_pdf(image_list, output_pdf_path):
    ...
    with open(output_pdf_path, "rb") as f:
        st.download_button("Download Processed PDF", f, "Output.pdf", "application/pdf")

mod = st.radio("Select your mode",['Replace Sign', 'Add Background Logo'])

if mod == "Replace Sign":
    msg = "Upload Signature Image"
...
if uploaded_file:
    ...
    if images:
        ...
        if st.session_state["rectangles"]:
            if mod == "Add Background Logo":
                opacity = st.slider("Select Signature Opacity", 0, 255, 128)
                r, g, b, alpha = esign.split()
                alpha = alpha.point(lambda p: p * (opacity / 255))
                esign = Image.merge("RGBA", (r, g, b, alpha))
